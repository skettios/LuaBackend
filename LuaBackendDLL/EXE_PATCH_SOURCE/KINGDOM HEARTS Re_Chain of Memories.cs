...
if (File.Exists("LuaBackend.dll"))
{
    if (!Directory.Exists(_scrFolder))
        MessageBox.Show("The \"" + _scrFolder + "\" folder was not found.\nLuaEngine will not operate.", 
                        "KINGDOM HEARTS Re: Chain of Memories", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);

    else
    {
        if (VersionLUA() > 128)
            MessageBox.Show("The LuaEngine DLL is too new for this executable.\nLuaEngine will not operate until this EXE is updated.", 
                            "KINGDOM HEARTS Re: Chain of Memories", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        {
            long _addr = Process.GetCurrentProcess().MainModule.BaseAddress.ToInt64() + 0x4E4660;
